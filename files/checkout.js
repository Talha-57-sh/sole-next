    ...
    function removeFromCart(i) { cart.splice(i, 1); renderCart(); saveCartToStorage(); if (!cart.length) document.getElementById('order').classList.remove('visible'); }
      ...
      if (!cart.length) {
        ...
        if(subV) subV.textContent = 'Rs 0.00';
        const grandV = document.getElementById('cart-grand-total-val');
        if(grandV) grandV.textContent = 'Rs 0.00';
        const delV = document.getElementById('cart-delivery-val2');
        if(delV) delV.textContent = 'Rs ' + deliveryCharge.toLocaleString() + '.00';
        return;
      }
      ...
      cart.forEach((item, i) => {
        ...
        html += `<div class="order-summary-item">
          ...
          <div class="os-price">Rs ${item.price.toLocaleString()}.00</div>
        </div>`;
      });
      ...
      if (discountRow) {
        if (discount > 0) {
          ...
          if (discountVal) discountVal.textContent = '- Rs ' + discount.toLocaleString() + '.00';
      ...
      const subV = document.getElementById('cart-subtotal-val');
      if(subV) subV.textContent = 'Rs ' + subtotal.toLocaleString() + '.00';
      const delV = document.getElementById('cart-delivery-val2');
      if(delV) delV.textContent = 'Rs ' + deliveryCharge.toLocaleString() + '.00';
      const shpV = document.getElementById('shipping-method-val');
      if(shpV) shpV.textContent = 'Rs ' + deliveryCharge.toLocaleString() + '.00';
      const grandV = document.getElementById('cart-grand-total-val');
      if(grandV) grandV.textContent = 'Rs ' + (subtotal + deliveryCharge - discount).toLocaleString() + '.00';
