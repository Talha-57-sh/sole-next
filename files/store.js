// Ensure page is visible immediately before anything else runs
        ...
        // =============================================
        // FIREBASE CONFIG — shared by store & admin
        // =============================================
        ...
        function initFirebase(config) {
            try {
                ...
                // Test connection using a public products read (no auth required on store page)
                db.collection('products').limit(1).get().then(() => {
                    ...
                    // Even if ping fails, still try listeners — Firestore rules allow product reads
                    dbReady = true;
                    setDbStatus('connected', '🔥 Firebase connected — data syncing in real-time');
                    startRealtimeListeners();
                });
            ...
            db.collection('settings').doc('store').onSnapshot(snap => {
                if (snap.exists) {
                    ...
                    if (data.returnPolicy) {
                        document.getElementById('policy-body').innerHTML = data.returnPolicy;
                    }
                    ...
                    if (typeof data.usdRate === 'number' && data.usdRate > 0) {
        ...
        function renderStore(filtered) {
            ...
            list.forEach((p, originalIndex) => {
                ...
                card.innerHTML =
                    ...
                    imgHTML +
                    ((p.tag === 'HDSBHSJD' || !p.tag) ? '<div class="product-tag">Sale</div>' : '<div class="product-tag">' + escHtml(p.tag) + '</div>') +
                    ...
                    '<button class="add-cart-btn" onclick="addToCart(' + i + ',\'' + key + '\',this)" ' + addDisabled + '>Add to Cart</button>' +
                    '</div>';
        ...
        function handleSearch() {
            ...
            if (clearX) clearX.classList.toggle('visible', query.length > 0);
            ...
            renderStore(query ? filtered : undefined);
            
            // Re-bind scroll reveal so new cards appear
            if (typeof initProductCardReveal === 'function') initProductCardReveal();
        }

        function clearSearch() {
            document.getElementById('store-search').value = '';
            var clearX = document.getElementById('search-clear-x');
      ...
      <div class="pay-sum-price">Rs. ${item.price.toLocaleString()}</div>
        ...
        // WHATSAPP CONFIG
        // =============================================
        let waNumber = localStorage.getItem('sole_wa_number') || '';
    ...
    ${anyRelated.length ? `<div class="pdm-related"><div class="pdm-related-title">You Might Also Like</div><div class="pdm-related-row">${relatedHtml}</div></div>` : ''}`;
      ...
      <div class="pdm-img-badge">${pdmImgIdx + 1} / ${imgs.length}</div>`;
        ...
        function renderCategoryBar() {
            ...
            let html = `<button class="cat-pill${activeCat === '' ? ' active' : ''}" onclick="filterCategory('')">All</button>`;
            cats.forEach(c => {
                html += `<button class="cat-pill${activeCat === c ? ' active' : ''}" onclick="filterCategory('${escHtml(c)}')">${escHtml(c)}</button>`;
            });
            bar.innerHTML = html;
        }
        ...
        function handleSearch() {
            const query = document.getElementById('store-search').value.trim().toLowerCase();
            const clearX = document.getElementById('search-clear-x');
            if (clearX) clearX.classList.toggle('visible', query.length > 0 || activeCat !== '');
            ...
            renderStore(query || activeCat ? filtered : undefined);
            
            // If scroll reveal is active, ensure new cards are visible
        ...
        function showOrderConfirmation(order) {
            ...
            itemsEl.innerHTML = (order.items || []).map(it => {
                const prod = products.find(p => p.name === it.name);
                const imgSrc = it.img || (prod && (prod.img || (prod.images && prod.images[0]))) || '';
                const emoji = EMOJIS[Math.abs((it.name || '').length % EMOJIS.length)];
                const thumb = imgSrc
                    ? `<img class="order-conf-thumb" src="${escHtml(imgSrc)}" alt="${escHtml(it.name)}" onerror="this.outerHTML='<div class=\'order-conf-thumb-emoji\'>${emoji}</div>'">`
                    : `<div class="order-conf-thumb-emoji">${emoji}</div>`;
                return `<div class="order-conf-item">${thumb}<div class="order-conf-item-info"><div class="order-conf-item-name">${escHtml(it.name)}</div><div class="order-conf-item-size">Size EU ${it.size}</div></div><div class="order-conf-item-price">Rs. ${it.price.toLocaleString()}</div></div>`;
            }).join('');

            // Totals
            const discount = order.discount || 0;
            let totalsHtml = `<div class="order-conf-total-row"><span>Subtotal</span><span>Rs. ${order.subtotal.toLocaleString()}</span></div>`;
            if (discount > 0) totalsHtml += `<div class="order-conf-total-row" style="color:var(--green)"><span>Discount${order.couponCode ? ' (' + order.couponCode + ')' : ''}</span><span>−Rs. ${discount.toLocaleString()}</span></div>`;
            totalsHtml += `<div class="order-conf-total-row"><span>🚚 Delivery</span><span>Rs. ${(order.deliveryCharge || 0).toLocaleString()}</span></div>`;
            totalsHtml += `<div class="order-conf-total-row grand"><span>Grand Total</span><span>Rs. ${order.total.toLocaleString()}</span></div>`;
            document.getElementById('conf-totals').innerHTML = totalsHtml;
            ...
            document.getElementById('conf-info-grid').innerHTML = [
            ...
            document.getElementById('conf-steps').innerHTML = steps.map(s =>
